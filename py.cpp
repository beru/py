...
#define INIT_SYMBOL(name) {&name, #name,},
...
bool pyLoadLib(const char* libPath, std::string* pError)
{
	...
	for (size_t i = 0; i < _countof(symbols); ++i) {
		auto& s = symbols[i];
		auto sym = ::GetProcAddress(hModule, s.name);
		*(void**)s.ptr = sym;
	}
	return true;
}

bool pyUnloadLib(std::string* pError)
{
	if (!hModule) {
		return false;
	}
